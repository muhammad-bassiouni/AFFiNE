...
const logger = new DebugLogger('affine:peer:socketio');
...
export function createAffinePeer(
  workspaceId: string
): CRDT & { disconnect: () => void } {
  logger.debug('createAffineDataSource', workspaceId);
  ...
  const onUpdate = new Slot<{ docId: string; data: Uint8Array }>();
  ...
  return {
    ...
    subscribe(cb) {
      return onUpdate.on(({ docId, data }) => cb(docId, data)).dispose;
    },
    disconnect() {
      syncSender.stop();
